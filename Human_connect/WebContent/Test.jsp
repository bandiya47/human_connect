		...
		<form name="test" method="post" action="volunteerDetail" encType="UTF-8">
			<div>
				<li>
					...
					센터 아이디 : ${volunteerList.c_id } <br>
					봉사 제목 : ${volunteerList.v_title } <br>
					봉사 날짜 : ${volunteerList.v_start_date } ~ ${volunteerList.v_end_date }<br>
					봉사 시간 : ${volunteerList.v_start_time } ~ ${volunteerList.v_last_time }<br>
					모집 날짜 : ${volunteerList.v_rstart_date } ~ ${volunteerList.v_rend_date }<br>
					모집 분야 : ${volunteerList.service_code }<br>
					모집 인원 : ${volunteerList.v_max_amnt }<br>
